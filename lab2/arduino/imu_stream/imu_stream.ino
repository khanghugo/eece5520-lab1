...
void initMPU6050()
{
  ...
  // KL EDIT: I have MPU-9200 and its WHO_AM_I is 0x70
  ...
  //   0b10 (0x10) → ±1000 °/s
  //   0b11 (0x18) → ±2000 °/s
  const uint8_t GYRO_CONFIG_BYTE = 0; // 0b00 there I guess...
  writeRegister(REG_GYRO_CONFIG, GYRO_CONFIG_BYTE);
  ...
  //   0b10 (0x10) → ±8 g
  //   0b11 (0x18) → ±16 g
  const uint8_t ACCEL_CONFIG_BYTE = 0; // it is all zero
  writeRegister(REG_ACCEL_CONFIG, ACCEL_CONFIG_BYTE);
}
...
uint8_t readRegister(uint8_t reg)
{
  ...
  // request a read from said register
  Wire.beginTransmission(MPU6050_ADDR);
  Wire.write(reg);
  Wire.endTransmission(false);

  // now actually requesting
...
void readAndSendIMU()
{
  // Part A: burst-read 14 bytes via I2C (see instructions above)
  float ax, ay, az, gx, gy, gz;
  int16_t raw_ax, raw_ay, raw_az, raw_gx, raw_gy, raw_gz;
  uint8_t buf[14];

  // cannot use the readRegister() here because it only reads 1 byte... smh....
  // ACCEL_XOUT_H is just the XOUT, but the document says it supports burst read
  // and the reader automatically increments the reader byte, i guess that's it then.
  // But the actual read value is a 16-bit values, not 8-bit.
  // But whatever, I guess just read everything and then deal with it later.
  Wire.beginTransmission(MPU6050_ADDR);
  Wire.write(REG_ACCEL_XOUT_H);
  Wire.endTransmission(false); // repeated START, keep bus
  ...
  // Part B: combine high/low bytes into int16_t raw values
  ...
  // Part C: convert raw values to physical units (float ax, ay, az, gx, gy, gz)
