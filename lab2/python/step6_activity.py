...
# ─── Tunable Thresholds ───────────────────────────────────────────────────────
THRESH_MOVING: float   = 0.03   # |mag-1.0| above this → not stationary  (g)
THRESH_SHAKING: float  = 0.45   # |mag-1.0| above this → shaking          (g)
STAT_WINDOW: int       = 8      # window size for stationary detection (samples)
...
class ActivityDetector:
    ...
    def _classify(self) -> ActivityState:
        ...
        # not enough to detect shake
        if len(devs) < 3:
            return self.state

        # check shake first because it is the easiest
        shake_window = devs[-SHAKE_COUNT:]        
        is_shake = len(list(filter(lambda x: x > THRESH_SHAKING, shake_window))) != 0

        if is_shake:
            self.state = ActivityState.SHAKING
            return self.state
        
        # not enough to detect stationary
        if len(devs) < 5:
            return self.state
        
        # at this state, it cannot be shaking anymore so
        # i dont need to worry about having weird values
        stationary_window = devs[-STAT_WINDOW:]
        is_stationary = all(map(lambda x: x <= THRESH_MOVING,stationary_window))

        if is_stationary:
            self.state = ActivityState.STATIONARY
        else:
