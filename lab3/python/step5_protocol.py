...
def encode_frame(
    ...
    Design and implement a binary framing protocol.
    ...
    """
    distance_bytes = int(distance_cm * 10).to_bytes(2, byteorder='big')
    motor_pwm_byte = int(motor_pwm).to_bytes(1)[0]
    alarm_state_byte = int(alarm_state).to_bytes(1)[0]
    zone_id_byte = int(zone_id).to_bytes(1)[0]
    checksum_byte = distance_bytes[0] ^ distance_bytes[1] ^ motor_pwm_byte ^ alarm_state_byte ^ zone_id_byte

    return bytes([FRAME_START, distance_bytes[0], distance_bytes[1], motor_pwm_byte, alarm_state_byte, zone_id_byte, checksum_byte])
...
def decode_frame(data: bytes) -> Optional[ParkingFrame]:
    ...
    distance_cm = int.from_bytes(data[1:3], byteorder='big') / 10 # undo multiplication
    motor_pwm = data[3]
    alarm_state = data[4]
    zone_id = data[5]
    checksum = data[6]
    ...
    return ParkingFrame(distance_cm=distance_cm, motor_pwm=motor_pwm, alarm_state=alarm_state, zone_id=zone_id, timestamp=time.time())
